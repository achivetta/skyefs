...
SkyeFS consists of a client (\code{skye\_\-client}) which functions as a
...
adaption of the \code{pvfs2fuse} code to use the lowlevel FUSE API.  However,
...
speed operation, the client also maintains a cache of \code{giga\_mapping}s
for recently accessed directories.

For performance and load-balance reasons, locality between the
\code{skye\_\-server} process and the PVFS server to which 
...
the client with a copy of its own \code{giga\_mapping} for the directory.  Upon receiving
...
to the new \code{skye\_\-server}, as per the updated mapping.  This
...
these operations to the \code{skye\_server}, the server can ensure that these
operations proceed 
...
\code{lookup} callback from FUSE, it consults its cached bitmap (if any) and then
...
the \code{skye\_server} provide a bootstrap bitmap to clients when returning a
\code{lookup()} result for a directory that is already cached on the server.
...
\end{figure}
The \code{skye\_server} provides a \code{create()} RPC for the
...
split.  This check is implemented using the PVFS \code{getattr} operation
...
three passes.  When the split is complete, a \code{bucket\_\-add} RPC is
...
asynchronously with respect to the triggering \code{create} operation.
...
the \code{rename} call, causing the source partition to change, the
\code{rename} will return \code{ENOENT}.  The client will then retry the
\code{partition} RPC to see if the partition's handle has changed.  If so, the
...
\end{figure}
To simplify \code{readdir} and ensure correct semantics, the
\code{skye\_client} implements the actual directory reading during the FUSE
\code{opendir} callback at which point the contents of the directory are loaded
into an array from which each subsequent \code{readdir} command is serviced.
...
to the PVFS cluster and restarting all \code{skye\_\-server} processes.
...
\code{make -j8}) would result in long stalls while executing PVFS calls.
...
The \code{pvfs2fuse} application distributed with PVFS provided valuable
