...
//static void struct GigaMapping_update_radix(struct GigaMapping *table);
...
//
void giga_update_cache(struct GigaMapping *curr, struct GigaMapping *update)
{
    int i;
...
// has been successful.
//
void giga_update_mapping(struct GigaMapping *mapping, index_t new_index)
{
...
// Print the struct GigaMapping contents. 
//
void giga_print_mapping(struct GigaMapping *mapping, FILE* fp)
{
    assert(mapping != NULL);
...
//
struct GigaMapping* giga_init_table()
{
    struct GigaMapping* table = (struct GigaMapping*)malloc(sizeof(struct GigaMapping));
    if (table == NULL)
    {
